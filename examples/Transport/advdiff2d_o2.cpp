...
int main()
{
    ...
    MESH mesh("msh/Square2D/mesh_tri_o2.dat");
    //scale and transpose mesh
    for (int i = 1; i <= mesh.TotalPoints; i++)
    {
        mesh.points[i].x = 2*mesh.points[i].x;
        mesh.points[i].y = 2*mesh.points[i].y;
        mesh.points[i].x = mesh.points[i].x - 1.0;
        mesh.points[i].y = mesh.points[i].y - 1.0;
    }
